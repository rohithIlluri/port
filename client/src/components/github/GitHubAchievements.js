...
const GitHubAchievements = ({ achievements }) => {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm p-6 border border-black/10 rounded-lg shadow-lg">
      <h4 className="text-xs font-medium uppercase tracking-wide text-black/80 mb-4">GitHub Achievements</h4>
      <div className="space-y-3">
        {achievements.map((achievement, index) => (
          <div key={index} className="flex items-center gap-3 p-3 bg-black/5 rounded-lg hover:bg-black/10 transition-all duration-300 hover:scale-[1.02] cursor-pointer">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                {achievement.tier && (
                  <span className="px-2 py-1 bg-black/10 text-xs font-medium text-black/70 rounded-md uppercase tracking-wide">
                    {achievement.tier}
                  </span>
                )}
              </div>
